...
function buildRows(groups: AnalyticsGroup[] | undefined): AgentRow[] {
  ...
  for (const g of groups) {
    ...
    if (!row) {
      row = { key, label: g.label, isSystem: g.isSystem, userCost: 0, systemCost: 0, total: 0 }
      byAgent.set(key, row)
    }
    if (g.isSystem) {
      row.systemCost += g.cost
      row.isSystem = true
    } else {
      row.userCost += g.cost
    }
              ...
              <div key={row.key}>
                <div className="flex items-center justify-between text-xs">
                  <span className="flex min-w-0 items-center gap-1.5">
                    ...
                    {row.isSystem && (
                      <span className="rounded bg-amber-500/10 px-1.5 py-0.5 text-[10px] font-medium text-amber-400">系统</span>
                    )}
                  </span>
