...
export const repos = pgTable(
  ...
  (t) => [uniqueIndex("repos_local_path_idx").on(t.localPath)],
)
...
export const milestones = pgTable(
  ...
  (t) => [uniqueIndex("milestones_repo_number_idx").on(t.repoId, t.number), index("milestones_repo_idx").on(t.repoId)],
)
...
export const promptFragments = pgTable(
  ...
  (t) => [index("prompt_fragments_repo_idx").on(t.repoId)],
)
...
export const customAgents = pgTable(
  ...
  (t) => [index("custom_agents_repo_idx").on(t.repoId)],
)

export const customAgentFragments = pgTable(
  "custom_agent_fragments",
  {
    ...
    fragmentId: text("fragment_id")
      .notNull()
      .references(() => promptFragments.id, { onDelete: "cascade" }),
    position: integer("position").notNull().default(0),
  },
  (t) => [primaryKey({ columns: [t.customAgentId, t.fragmentId] })],
)
...
export const messages = pgTable(
  ...
  (t) => [index("messages_session_idx").on(t.sessionId, t.timeCreated)],
)
...
export const gitHosts = pgTable(
  ...
  (t) => [uniqueIndex("git_hosts_host_idx").on(t.host)],
)
...
export const issueComments = pgTable(
  ...
  (t) => [index("issue_comments_issue_idx").on(t.issueId), index("issue_comments_repo_idx").on(t.repoId)],
)
...
export const tags = pgTable(
  ...
  (t) => [uniqueIndex("tags_repo_name_idx").on(t.repoId, t.name), index("tags_repo_idx").on(t.repoId)],
)
...
export const issueTags = pgTable(
  ...
  (t) => [primaryKey({ columns: [t.issueId, t.tagId] }), index("issue_tags_tag_idx").on(t.tagId)],
)
...
export const pullRequests = pgTable(
  "pull_requests",
  {
    ...
    diffStats:
      jsonb("diff_stats").$type<Array<{ filename: string; status: string; additions: number; deletions: number }>>(),
    createdAt: bigint("created_at", { mode: "number" }).notNull(),
    updatedAt: bigint("updated_at", { mode: "number" }).notNull(),
...
export const prIssueLinks = pgTable(
  ...
  (t) => [primaryKey({ columns: [t.prId, t.issueId] }), index("pr_issue_links_issue_idx").on(t.issueId)],
)
