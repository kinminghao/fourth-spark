...
import { createGitIssueClient, getHostInfo, GitApiError } from "../lib/git-provider"
import { issueId, issueToDb, commentToDb, extractUpstreamMessage, rewriteAttachmentUrls, getRepoGitClient } from "../lib/git-utils"
...
function issueCreateDraftPath(repoId: string): string {
...
issueRoutes.get("/draft-create", async (c) => {
  const repoId = c.req.param("repoId")!
...
issueRoutes.delete("/draft-create", async (c) => {
  const repoId = c.req.param("repoId")!
  const filePath = issueCreateDraftPath(repoId)
  await unlink(filePath).catch(() => {})
  return c.json({ ok: true })
