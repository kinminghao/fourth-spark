...
import { findDockerCompose, getDockerComposeCmd, isProcessRunning, PID_FILE } from "./paths"
...
export async function stopCommand(): Promise<void> {
  if (existsSync(PID_FILE)) {
    ...
    try {
      ...
      try {
        execSync("pkill -x fourth-spark", { stdio: "pipe" })
      } catch {}
      await new Promise((r) => setTimeout(r, 1000))
      try {
        execSync("pkill -9 -x fourth-spark", { stdio: "pipe" })
      } catch {}
      console.log("  Server stopped")
    } catch {
  ...
  try {
    ...
    try {
      execSync("pkill -f 'opencode serve --port'", { stdio: "pipe" })
    } catch {}
    console.log("  Done")
  } catch {
